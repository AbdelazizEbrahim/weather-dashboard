...
import { ArrowLeft, Thermometer, Droplets, Wind, Eye, ToggleLeft, ToggleRight } from "lucide-react"
import { fetchWeather, toggleUnit, clearError } from "@/lib/features/weather/weatherSlice"
...
export default function CityWeatherPage() {
  ...
  const { weather, loading, error, unit, cache } = useSelector((state: RootState) => state.weather)
  ...
  useEffect(() => {
    if (decodedCity) {
      ...
      const cachedData = cache[decodedCity.toLowerCase()]
  ...
  if (error) {
    return (
      <div className="min-h-screen flex items-center justify-center p-4">
        <ErrorMessage error={error} onRetry={() => dispatch(fetchWeather(decodedCity))} onBack={handleBack} />
      </div>
    )
  }
    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-white text-center">
          ...
          <button onClick={handleBack} className="mt-4 text-white/80 hover:text-white">
            ← Back to search
          </button>
        </div>
      </div>
  ...
  return (
    <div className="min-h-screen p-4">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="bg-white/10 backdrop-blur-md rounded-3xl p-8 shadow-2xl border border-white/20 mb-6">
          ...
          <div className="flex items-center justify-center mb-8">
            <div className="text-center">
              ...
              <div className="text-6xl font-bold text-white mb-2">
                {convertTemperature(weather.main.temp)}°{unit === "celsius" ? "C" : "F"}
              </div>
              <p className="text-white/80 text-xl capitalize">{weather.weather[0].description}</p>
              <p className="text-white/60 text-sm mt-2">
                Feels like {convertTemperature(weather.main.feels_like)}°{unit === "celsius" ? "C" : "F"}
              </p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div className="bg-white/10 rounded-xl p-4 text-center">
              ...
              <p className="text-white font-semibold">
                {convertTemperature(weather.main.temp_min)}° / {convertTemperature(weather.main.temp_max)}°
              </p>
            </div>
          ...
          <div className="mt-6 grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="bg-white/10 rounded-xl p-4">
              <h3 className="text-white font-semibold mb-2">Atmospheric Pressure</h3>
        ...
        {/* Last Updated */}
        <div className="text-center text-white/60 text-sm">Last updated: {new Date().toLocaleString()}</div>
      </div>
    </div>
  )
}
